...
namespace Nzh.Hero.Service
{
    public class SysRoleService : BaseService, ISysRoleService
    {
        ...
        {
            int total = 0;
            //var query = Sqldb.Queryable<sys_role>().OrderBy(s => s.sort).ToPageList(param.page, param.limit, ref total);
        ...
        {
            //var menu = Sqldb.Queryable<sys_menu>().OrderBy(s => s.menu_sort).Select(s => new ZtreeDto(){id = s.id.ToString(),name = s.menu_name,pId = s.parent_id.ToString()}).ToList();
            ...
            var func = Sqldb.Queryable<sys_menu_ref_operate, sys_operate, sys_menu>((f, o, m) => f.operate_id == o.id && f.menu_id == m.id).Select((f, o, m) => new ZtreeDto() { id = f.operate_id.ToString(), pId = f.menu_id.ToString(), name = o.func_cname }).ToList();
            menu.AddRange(func);
            //var role =Sqldb.Queryable<sys_role_authorize>().Where(s => s.role_id == SqlFunc.ToInt64(roleid)).Select(s => new { s.menu_id, s.menu_pid }).ToList();
